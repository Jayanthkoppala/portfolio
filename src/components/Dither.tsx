...
const RetroEffect = forwardRef(function RetroEffect(
  { colorNum, pixelSize }: RetroEffectProps,
  ref: ForwardedRef<RetroEffectImpl>,
) {
  const effect = useMemo(
    () => new RetroEffectImpl({ colorNum, pixelSize }),
    [colorNum, pixelSize],
  );
...
function DitheredWaves({
  ...
  const previousColor = useRef<WaveColor>([...waveColor]);
  ...
  useEffect(() => {
    ...
    const materialResolution = materialRef.current?.uniforms.resolution
      ?.value as THREE.Vector2 | undefined;
    materialResolution?.set(width, height);
  }, [gl, size]);

  useFrame(({ clock }) => {
    const uniforms = (materialRef.current?.uniforms ??
      waveUniformsRef.current) as typeof waveUniformsRef.current;
...
export default function Dither({
  ...
  const staticFrame = disableAnimation || !active;

  return (
    <div
      ref={hostRef}
      data-animation-active={staticFrame ? "false" : "true"}
    ...
    >
      ...
      >
        <DitheredWaves
          ...
          colorNum={colorNum}
          pixelSize={pixelSize}
          disableAnimation={staticFrame}
          enableMouseInteraction={enableMouseInteraction && active}
